...
**12-class 결함 semantic segmentation** 모델 (v1).

**DINOv2 (frozen) backbone + DPT-style multi-scale decoder** 를
**2-stage(KD pretrain → GT finetune)** 로 학습한다.

> 공유 코드(모델·손실·샘플러·로깅·이미지 유틸·공통 상수)는 [../common/](../common/) 에 있다.
> 8-class + cross-val + DSCNN aug 변종은 [../v2/](../v2/) 참고.
> 학습 중 만난 이슈/해결 기록은 [DEBUG_HISTORY.md](DEBUG_HISTORY.md), 클래스 정의는 [../paper/DSCNN/DSCNN_Classes.md](../paper/DSCNN/DSCNN_Classes.md) 참고.
...
- 재료별 native → ORNL 매핑은 [../common/config.py](../common/config.py) 의 `MATERIAL_TO_ORNL` 참조.
...
DefSeg_AM/
├── common/                    # ★ v1·v2 공유 코드
│   ├── config.py              # 경로·ORNL 라벨공간·MATERIAL_TO_ORNL·backbone·출력경로
│   ├── data/{image_utils.py, samplers.py}
│   ├── models/{model.py, losses.py}
│   ├── training/dist_utils.py
│   └── utils/log.py
│
├── v1/                        # ← 본 폴더 (12-class)
│   ├── config.py              # v1 전용 하이퍼파라미터 (S1_*/S2_*) + common re-export
│   ├── data/
...
│   ├── training/{train_stage1.py, train_stage2.py}
│   ├── inference/{infer.py, confusion.py}
│   ├── scripts/               # nohup 백그라운드 실행 래퍼
│   ├── docs/
│   └── PLAN.md / DEBUG_HISTORY.md
│
├── v2/                        # 8-class + cross-val + DSCNN aug 변종
│
├── cache/ checkpoints/ figures/ logs/   # (gitignore) v1·v2 공유 출력 (run-name 으로 구분)
└── paper/ requirements.txt
```

> 공유 모델·손실·샘플러·로깅·이미지 유틸은 모두 `../common/` 에 있고, v1·v2 가 함께 임포트한다.
...
`python -m DefSeg_AM.v1.<module>` 로 실행.
GPU 선택은 스크립트 안의 `CUDA_VISIBLE_DEVICES` 로 조정.

### (0) 캐시 사전 생성 — 학습 전 1회 필수

```bash
bash DefSeg_AM/v1/scripts/run_build_cache.sh
```
...
```bash
bash DefSeg_AM/v1/scripts/run_stage1.sh
...
```bash
bash DefSeg_AM/v1/scripts/run_stage2.sh   # 동일 run-name 의 stage1_best.pt 자동 load
...
```bash
./DefSeg_AM/venv/bin/python -m DefSeg_AM.v1.inference.infer \
