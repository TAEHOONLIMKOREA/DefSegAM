"""v3 Augmentation — PLAN_v4 §3 + DSCNN_Summary §6.
...
사용:
    from DefSeg_AM.v4.data.augmentation import augment_v4
    i0, i1, ann = augment_v4(i0, i1, ann, training=True)
"""
...
def augment_v4(
    ...
    """v4 augmentation 종합 적용 — v3 augmentation + Copy-Paste 추가.

    Args:
        i0, i1: uint8 (H, W) — after-melt / after-spread images
        ann:    int8  (H, W) — label (11-class + IGNORE=-1)
        training: True 면 random aug 적용. False 면 통과.
        rng: 외부 RNG (재현성 필요 시).
        cp_sampler: data.copy_paste.CopyPasteSampler 인스턴스 (Stage 1/2 별로 다른 supplier).
            None 이면 Copy-Paste 적용 안 함.

    적용 순서 (PLAN_v4 §6.3):
        Copy-Paste → cyclic shift → flip LR/UD → rot180 → DSCNN noise/intensity → brightness
    ...
    # 0. Copy-Paste (rare class object 합성) — 가장 먼저 적용해 후속 기하 augmentation 으로 다양화
    if cp_sampler is not None and cfg.CP_ENABLE:
        i0, i1, ann = cp_sampler.paste(i0, i1, ann, rng)
