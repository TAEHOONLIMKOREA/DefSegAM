"""v2 Stage 1 학습 — ORNL HDF5 (DSCNN pred) KD pretrain, 8-class.

v1 train_stage1 의 helper (init_distributed, update_counts, reduce_counts) 와
모델 (DefSegModel), 공통 sampler 등 그대로 재사용.

추가 사항 (PLAN_v2 §5.1, §6 + DSCNN_Summary §5.6, §12):
  - 8-class 출력 (config_v2.N_CLASSES_V2 = 8)
  - v2 dataset (image v1 cache + label v2 cache) + augmentation 통합
...
사전조건:
    python -m DefSeg_AM.v2.data.build_cache_v2     # label 만 v2 8-class 재빌드

사용:
    python -m DefSeg_AM.v2.training.train_stage1 [--use-hard-bootstrap] [--quick]
"""
...
from ..data.data_ornl_v2 import (
    DefSegORNLCachedDatasetV2,
    estimate_class_counts_v2,
)
from ..models.losses_v2 import hard_bootstrap_loss, median_inv_class_weight


# ---------------------------------------------------------------------------
# v2 의 update_counts (N_CLASSES_V2=8 사용 — v1 의 update_counts 는 config.N_CLASSES=12)
# ---------------------------------------------------------------------------

@torch.no_grad()
def update_counts_v2(
...
def main():
    ...
    ap.add_argument("--oversample-power", type=float, default=cfg.S1_OVERSAMPLE_POWER)
    ...
    ap.add_argument(
        ...
        help="DSCNN 원본 (median/freq) 도 옵션 (PLAN_v2 §6.E)",
    )
    ...
    log = setup_logger(rank=rank, name="stage1_v2")
    device = torch.device(f"cuda:{local_rank}" if torch.cuda.is_available() else "cpu")

    if is_main(rank):
        log.info(f"=== v2 stage1 KD pretrain ===  world_size={world_size}  run_name={args.run_name}")
        log.info(f"args: {vars(args)}")
        log.info(f"classes: {cfg.ORNL_CLASS_NAMES_V2}")
    ...
    # ----- v2 Dataset (image v1 cache + label v2 cache + augmentation) -----
    train_ds = DefSegORNLCachedDatasetV2("train", args.img_size, training=True)
    val_ds = DefSegORNLCachedDatasetV2("val", args.img_size, training=False)
    if is_main(rank):
        log.info(f"train={len(train_ds)} layers, val={len(val_ds)} layers (v2 cache)")

    # ----- Train sampler: WeightedRandomSampler with v2 defect_ratio -----
    sampler_weights = train_ds.defect_ratios ** args.oversample_power + cfg.S1_OVERSAMPLE_EPS
    ...
    # ----- Class weights (v2 8-class 기반) -----
    if is_main(rank):
        log.info(f"estimating class weights from {args.n_weight_sample} sampled layers …")
    counts = estimate_class_counts_v2(train_ds, n_sample=args.n_weight_sample)
