"""v3 label cache 빌드 — v1 image cache 의 label.npy 를 v3 10-class 로 재매핑.

v1 image cache (visible_0.npy, visible_1.npy) 는 그대로 사용.
v1 label.npy 의 각 픽셀 값 (0..11) 을 ORNL_12_TO_NEW_10 로 재매핑하여 label_v3.npy 로 저장.

기존 v1 cache 가 없으면 ORNL HDF5 부터 재빌드해야 함 (build_cache_stage1 사용).

사용:
    python -m DefSeg_AM.v3.data.build_cache_v3 [--img-size 1036] [--rebuild]

산출:
    cache/resized_sz1036_v3/<build>/
    ├── label_v3.npy           (n_layers, IMG, IMG) int8
    └── meta.npz               (orig_layer_idxs, defect_ratios_v3)
    cache/resized_sz1036_v3/{train,val}_index.npz
"""
...
def remap_one_build(
    ...
    """한 build 의 label.npy 를 v3 로 재매핑하여 저장.

    Returns:
        meta dict: {n_layers, defect_ratios_v3 (per layer, class 2..7 픽셀 비율)} 또는 None (skip)
    """
    v1_bdir = v1_dir / build_id
    v3_bdir = v3_dir / build_id
    v3_label = v3_bdir / "label_v3.npy"
    v3_meta = v3_bdir / "meta.npz"
    ...
    if v3_label.exists() and v3_meta.exists() and not rebuild:
        log.info(f"{build_id}: v3 label cache 이미 존재, skip (--rebuild 로 강제 가능)")
        d = np.load(v3_meta, allow_pickle=False)
        return {
            "n_layers": int(d.get("n_layers", 0)) if "n_layers" in d.files else None,
            "defect_ratios_v3": d["defect_ratios_v3"],
            "orig_layer_idxs": d["orig_layer_idxs"],
        }

    v3_bdir.mkdir(parents=True, exist_ok=True)
    ...
    out_mm = np.lib.format.open_memmap(
        v3_label, mode="w+", dtype=np.int8, shape=(n, H, W),
    )
    ...
    for start in range(0, n, chunk):
        ...
        block_v3 = _remap_label_arr(block)
        out_mm[start:end] = block_v3
        # v3 defect 비율 계산 (class 2..9)
        for c in cfg.DEFECT_CLASS_INDICES_V3:
            defect_pix_per_layer[start:end] += (block_v3 == c).sum(axis=(1, 2))
        log.info(f"  {build_id} [{end:5d}/{n}] elapsed {(time.time()-t0)/60:.2f}m")
    out_mm.flush()
    del out_mm, lab_v1_mm

    defect_ratios_v3 = (defect_pix_per_layer / total_pix).astype(np.float32)

    np.savez(
        v3_meta,
        orig_layer_idxs=orig_layer_idxs,
        defect_ratios_v3=defect_ratios_v3,
...
def build_split_index(
    ...
    for build_id in builds:
        meta = remap_one_build(build_id, v1_dir, v3_dir, rebuild, log)
        if meta is None:
            continue
        n = meta["n_layers"]
        agg_build_ids.extend([build_id] * n)
        agg_cache_rows.extend(range(n))
        agg_defect_ratios.extend(meta["defect_ratios_v3"].tolist())

    np.savez(
        v3_dir / f"{split}_index.npz",
...
def main():
    ...
    ap.add_argument("--rebuild", action="store_true",
                    help="강제 재빌드 (기존 v3 label cache 무시)")
    ap.add_argument("--split", choices=["both", "train", "val"], default="both")
    args = ap.parse_args()

    log = setup_logger(rank=0, name="build_cache_v3")
    v1_dir = cfg.v1_cache_dir(args.img_size)
    v3_dir = cfg.v3_cache_dir(args.img_size)
    v3_dir.mkdir(parents=True, exist_ok=True)
    log.info(f"v1 image cache: {v1_dir}")
    log.info(f"v3 label cache: {v3_dir}")

    t_all = time.time()
    if args.split in ("both", "train"):
        build_split_index("train", cfg.ORNL_TRAIN_BUILDS, v1_dir, v3_dir, args.rebuild, log)
    if args.split in ("both", "val"):
        build_split_index("val", cfg.ORNL_VAL_BUILDS, v1_dir, v3_dir, args.rebuild, log)
