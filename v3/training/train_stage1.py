...
추가 사항 (PLAN_v3 §4.1, §6 + DSCNN_Summary §5.6, §12):
  - 10-class 출력 (config_v3.N_CLASSES_V3 = 10)
  - v3 dataset (image v1 cache + label v3 cache) + augmentation 통합
...
사전조건:
    python -m DefSeg_AM.v3.data.build_cache_v3     # label 만 v3 10-class 재빌드

사용:
    python -m DefSeg_AM.v3.training.train_stage1 [--use-hard-bootstrap] [--quick]
"""
...
from ..data.data_ornl_v3 import (
    DefSegORNLCachedDatasetV3,
    estimate_class_counts_v3,
)
from ..models.losses_v3 import hard_bootstrap_loss, median_inv_class_weight


# ---------------------------------------------------------------------------
# v3 의 update_counts (N_CLASSES_V3=10 사용 — v1 의 update_counts 는 config.N_CLASSES=12)
# ---------------------------------------------------------------------------

@torch.no_grad()
def update_counts_v3(
...
def main():
    ...
    ap.add_argument(
        ...
        help="DSCNN 원본 (median/freq) 도 옵션 (PLAN_v3 §6.E)",
    )
    ...
    log = setup_logger(rank=rank, name="stage1_v3")
    device = torch.device(f"cuda:{local_rank}" if torch.cuda.is_available() else "cpu")

    if is_main(rank):
        log.info(f"=== v3 stage1 KD pretrain ===  world_size={world_size}  run_name={args.run_name}")
        log.info(f"args: {vars(args)}")
        log.info(f"classes: {cfg.ORNL_CLASS_NAMES_V3}")
    ...
    # ----- v3 Dataset (image v1 cache + label v3 cache + augmentation) -----
    train_ds = DefSegORNLCachedDatasetV3("train", args.img_size, training=True)
    val_ds = DefSegORNLCachedDatasetV3("val", args.img_size, training=False)
    if is_main(rank):
        log.info(f"train={len(train_ds)} layers, val={len(val_ds)} layers (v3 cache)")

    # ----- Train sampler: WeightedRandomSampler with v3 defect_ratio -----
    ...
    # ----- Class weights (v3 10-class 기반) -----
    if is_main(rank):
        log.info(f"estimating class weights from {args.n_weight_sample} sampled layers …")
    counts = estimate_class_counts_v3(train_ds, n_sample=args.n_weight_sample)
