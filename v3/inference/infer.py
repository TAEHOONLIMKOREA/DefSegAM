"""v3 학습된 모델로 ORNL HDF5 layer 추론 → 4-panel 비교 PNG.

panel 구성: [visible/0, visible/1, DSCNN GT argmax (10-class 재매핑), our v3 prediction]
...
사용 예:
    ...
    python -m DefSeg_AM.v3.inference.infer --run-name <run> --stage 1

    # Stage 2 의 단일 ckpt
    python -m DefSeg_AM.v3.inference.infer --run-name <run> --stage 2 --layers 1500
...
from ..data.data_ornl_v3 import ornl_segmentation_argmax_v3
from .postprocess import apply_postprocess


# v3 10-class 팔레트
_PALETTE_V3 = np.array([
...
def _tta_v3_prob(
    ...
    """v3 학습 augmentation 일치 TTA: flip(LR) × flip(UD) × rot180 = 8-way 평균."""
    probs = []
...
def infer_layers_to_dir(
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        for li in layers:
            ...
            gt = ornl_segmentation_argmax_v3(seg, li)

            if use_tta:
                prob = _tta_v3_prob(model, i0, i1, img_size, device)
            else:
            ...
            if postprocess:
                ...
                print(
                    f"  layer {li}: pp se_swelling_relabeled={s['se_swelling_relabeled']:>7d}"
                )
...
def visualize(
    ...
    axes[2].imshow(colorize(gt_label));   axes[2].set_title("DSCNN GT (v3 10-class argmax)")
    axes[3].imshow(colorize(pred_label)); axes[3].set_title("our prediction (DefSeg-AM v3)")
    for ax in axes:
        ax.set_xticks([]); ax.set_yticks([])
    handles = [
        patches.Patch(color=tuple(_PALETTE_V3[c]),
                      label=f"{c} {cfg.ORNL_CLASS_NAMES_V3[c]}")
        for c in range(cfg.N_CLASSES_V3)
    ]
...
def _load_model_from_ckpt(ckpt_path: Path, device: torch.device) -> DefSegModel:
    ...
    model = DefSegModel(
        ...
        n_classes=c.get("n_classes", cfg.N_CLASSES_V3),
    ).to(device)
...
def main():
    ...
    if args.out_dir is None:
        ...
        out_dir = cfg.FIGURE_DIR / args.run_name / "v3" / tag / "inference" / args.build.replace(" ", "_")
    else:
        out_dir = Path(args.out_dir)

    print(f"build {args.build}: n_layers={n_total}, layers={layers}, out={out_dir}")
    title_prefix = (
        f"{args.build} / v3-stage{args.stage}"
