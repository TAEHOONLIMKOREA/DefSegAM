"""v3 DefSegModel 의 confusion matrix 평가 (학습 없음, 1-pass) — 10-class.

v3 는 cross-validation 이 없고 stage 2 도 단일 ckpt 라 fold/cv 옵션 없음.
평가 대상은 ORNL v3 cached split (Stage 1/2 모두 동일).

평가 모드:
  - --stage 1  : stage1_best.pt
  - --stage 2  : stage2_best.pt   (default)

출력 (기본 figures/<run>/v3/<tag>/confusion/):
...
def ornl_loader(split: str, img_size: int, batch_size: int, num_workers: int):
    ...
    ds = DefSegORNLCachedDatasetV3(split, img_size, training=False)
    builds = "Build 1" if split == "val" else "Builds 2-5"
    desc = f"ORNL v3 cached {split} ({builds}) — {len(ds)} layers"
...
def main():
    ...
    ap.add_argument("--run-name", type=str, default="vits14_dpt_dual_sz1036_10cls_v3")
    ...
    names = cfg.ORNL_CLASS_NAMES_V3
    n_classes = cfg.N_CLASSES_V3
    fig_root = cfg.FIGURE_DIR / args.run_name / "v3"
    ...
    dump_all(cm, out_dir, names, title=f"v3 stage{args.stage} ({args.split})")
